window.currentShaderFill = 1.0;
window.shaderZoom = 1.0; 
...
let shaderBrickScale = 1.2; 
let shaderBrickGap = 0.95;
let shaderExtrusion = 1.0;
let shaderFluidityFade = 0.0; // NEW
...
let _qtHrX, _qtHrY, _qtHrW, _qtHrH, _qtImgW, _qtImgH, _qtRefStep;
let _shaderUseUploadSource = false;
let _shaderBuildPending = false;
let _lastShaderWidth = 0, _lastShaderHeight = 0;
let _lastShaderUpload = null, _lastShaderWholePage = null;

// --- NEW FLUIDITY HELPER ---
function getShaderFluidityScale(x, y) {
  if (typeof shaderFluidityFade === 'undefined' || shaderFluidityFade <= 0.01) return 1.0;
  var n = noise(x * 0.003, y * 0.003);
  var minScale = 1.0 - shaderFluidityFade;
  var scale = map(n, 0.2, 0.8, minScale, 1.1); 
  return constrain(scale, 0.0, 1.0); 
}

function shaderExtrudeCapCubes() {
  const ext = typeof shaderExtrusion === 'number' ? constrain(shaderExtrusion, 0.12, 2.5) : 1;
...
function sampleInkAtCell(px, py, hrX, hrY, hrW, hrH, imgW, imgH) {
  ...
  if ((!_shaderUseUploadSource && isWholePageMode) || !src || !src.pixels || src.pixels.length < imgW * imgH * 4) return 0.45;
  let u = (px - hrX) / max(hrW, 1);
  let v = (py - hrY) / max(hrH, 1);
  u = u < 0 ? 0 : (u > 1 ? 1 : u); v = v < 0 ? 0 : (v > 1 ? 1 : v);
  const ix = (u * (imgW - 1)) | 0; const iy = (v * (imgH - 1)) | 0;
...
function distToTransparentEdge(px, py, maskPixels, mw, mh, maxR) {
  const px_i = px | 0; const py_i = py | 0;
  if (!maskPixelIsLogo(maskPixels, mw, mh, px_i, py_i)) return 0;

  for (let r = 2; r <= maxR; r += 2) { 
    const qh = px_i - r; const qj = px_i + r; const qk = py_i - r; const qm = py_i + r;
...
function flattenShaderExtrusionOutliers(blocks) {
  ...
  const cap = shaderExtrudeCapCubes();
  const orig = [];
  for (let i = 0; i < nb; i++) orig.push(constrain(floor(blocks[i].numCubes), LOGO_MIN_CUBES, cap));
  ...
  const grid = new Map();
  for (let i = 0; i < nb; i++) {
    const k = blocks[i].gx + ',' + blocks[i].gy; const v = orig[i];
    if (!grid.has(k) || grid.get(k) < v) grid.set(k, v);
  }
  for (let i = 0; i < nb; i++) {
    const gx = blocks[i].gx; const gy = blocks[i].gy;
...
function cellIntersectsLogo(maskPx, mw, mh, x0, y0, x1, y1) {
  ...
  const nx = min(12, max(2, 2 + floor(w / 10))); const ny = min(12, max(2, 2 + floor(h / 10)));
  const stepX = w / nx; const stepY = h / ny;
  for (let j = 0; j < ny; j++) {
...
function expandSquareBounds(bb, pad, mw, mh) {
  const bw = bb.maxX - bb.minX; const bh = bb.maxY - bb.minY;
  const side = max(bw, bh) + pad * 2;
  const cx = (bb.minX + bb.maxX) * 0.5; const cy = (bb.minY + bb.maxY) * 0.5;
  let x0 = floor(cx - side * 0.5); let y0 = floor(cy - side * 0.5);
  x0 = constrain(x0, 0, mw - 2); y0 = constrain(y0, 0, mh - 2);
  let x1 = floor(x0 + side); let y1 = floor(y0 + side);
  x1 = min(mw, max(x1, x0 + 8)); y1 = min(mh, max(y1, y0 + 8));
  return { x0, y0, x1, y1 };
}
...
function emitUniformVoxelGridBlocks(maskPx, mw, mh, region, cellPx, outBlocks, fillAllCells) {
  ...
  for (let y0 = yStart; y0 < region.y1; y0 += g) {
    for (let x0 = xStart; x0 < region.x1; x0 += g) {
      const x1 = min(mw, x0 + g); const y1 = min(mh, y0 + g);
      if (x1 <= x0 || y1 <= y0) continue;

      const touchesLogo = cellIntersectsLogo(maskPx, mw, mh, x0, y0, x1, y1);
      if (!touchesLogo && !tileAll) continue;

      const cwCell = x1 - x0; const chCell = y1 - y0;
      const pxGeo = x0 + cwCell * 0.5; const pyGeo = y0 + chCell * 0.5;

      let numCubes;
      if (!touchesLogo && tileAll) {
        numCubes = LOGO_MIN_CUBES;
      } else {
        let anchorPx = pxGeo; let anchorPy = pyGeo;
        let cxi = floor(constrain(anchorPx, x0, x1 - 1)); let cyi = floor(constrain(anchorPy, y0, y1 - 1));

        if (!maskPixelIsLogo(maskPx, mw, mh, cxi, cyi)) {
          let found = false; const nx = 8; const ny = 8;
          outer: for (let j = 0; j < ny; j++) {
            ...
            for (let ii = 0; ii < nx; ii++) {
              ...
              if (maskPixelIsLogo(maskPx, mw, mh, fx, fy)) {
                anchorPx = fx + 0.5; anchorPy = fy + 0.5; found = true; break outer;
              }
            }
          }
          if (!found) { if (tileAll) numCubes = LOGO_MIN_CUBES; else continue; }
        }

        if (numCubes === undefined) {
          cxi = floor(constrain(anchorPx, x0, x1 - 1)); cyi = floor(constrain(anchorPy, y0, y1 - 1));
          const ink = isWholePageMode && !_shaderUseUploadSource
              ? noise(floor(cxi / max(8, _qtRefStep)) * 0.17, floor(cyi / max(8, _qtRefStep)) * 0.19) * 0.55 + 0.28
      ...
      if (tileAll && typeof window.uploadSparsitySkipCell === 'function' && window.uploadSparsitySkipCell(floor(x0 / g), floor(y0 / g), 11)) continue;

      outBlocks.push({ cx: pxGeo - mw * 0.5, cy: pyGeo - mh * 0.5, cell: g, numCubes, gx: floor(x0 / g), gy: floor(y0 / g) });
    }
  }
}
...
function rebuildShaderGeometry() {
  ...
  if (_lastShaderWidth === width && _lastShaderHeight === height && _lastShaderUpload === uploadedSourceImg && _lastShaderWholePage === isWholePageMode && shaderBlocks.length > 0) return; 

  _lastShaderWidth = width; _lastShaderHeight = height; _lastShaderUpload = uploadedSourceImg; _lastShaderWholePage = isWholePageMode;
  shaderBlocks = [];
  ...
  const scale = typeof shaderBrickScale === 'number' ? constrain(shaderBrickScale, 0.45, 10.35) : 1.2;
  ...
  let imgW = max(logoImg.width, 1); let imgH = max(logoImg.height, 1);
  let hrW, hrH, hrX, hrY;
  if (useUploadMask && typeof getUploadDrawRect === 'function' && typeof uploadedSourceImg !== 'undefined' && uploadedSourceImg && uploadedSourceImg.width > 0) {
    const r = getUploadDrawRect(uploadedSourceImg);
    hrX = r.x; hrY = r.y; hrW = r.w; hrH = r.h;
  } else {
    let baseScale = min(width / imgW, height / imgH) * 0.55;
    let minScale = 280 / imgW; let scaleFactor = max(baseScale, minScale);
    hrW = floor(imgW * scaleFactor); hrH = floor(imgH * scaleFactor);
    hrX = floor((width - hrW) / 2); hrY = floor((height - hrH) / 2);
  }

  const mask = createGraphics(width, height);
  mask.pixelDensity(1); mask.clear();
  if (!isWholePageMode || useUploadMask) { mask.image(logoImg, hrX, hrY, hrW, hrH); } else { mask.background(255); }
  mask.loadPixels();
  const maskPx = mask.pixels; const mw = width; const mh = height;
  const brickGap = typeof shaderBrickGap === 'number' ? constrain(shaderBrickGap, 0.78, 0.995) : 0.97;
  const refStep = step * brickGap;

  _qtHrX = hrX; _qtHrY = hrY; _qtHrW = hrW; _qtHrH = hrH; _qtImgW = imgW; _qtImgH = imgH; _qtRefStep = refStep;

  const bb = maskBoundingBox(maskPx, mw, mh);
  if (!bb) { mask.remove(); return; }

  const pad = max(floor(refStep * 1.2), 8);
  const root = expandSquareBounds(bb, pad, mw, mh);
  const gridCell = max(6, floor(refStep));

  emitUniformVoxelGridBlocks(maskPx, mw, mh, { x0: 0, y0: 0, x1: mw, y1: mh }, gridCell, shaderBlocks, true);
  if (!useUploadMask && !isWholePageMode && shaderBlocks.length > 0) flattenShaderExtrusionOutliers(shaderBlocks);

  mask.remove();
}

function setupShader() {
  if (!shaderCanvas) shaderCanvas = createGraphics(width, height, WEBGL);
...
function drawShader() {
  if (_shaderBuildPending) { rebuildShaderGeometry(); _shaderBuildPending = false; }
  ...
  const zf = typeof window.shaderZoom === 'number' ? window.shaderZoom : 1.0;
  const fov = PI / 3.0;
  ...
  shaderCanvas.push();
  shaderCanvas.rotateX(0); shaderCanvas.rotateY(0);

  for (let i = 0; i < shaderBlocks.length; i++) {
    const b = shaderBlocks[i];
    
    // -- Fluidity Modulation --
    // We map cx/cy back to screen coordinates roughly
    const screenX = b.cx + cw * 0.5;
    const screenY = b.cy + ch * 0.5;
    let fScale = getShaderFluidityScale(screenX, screenY);
    
    // Sparsity Check from older Fill Density implementation, adapted to fade gracefully
    if (fScale < 0.05) continue;
    ...
    // Apply alpha fading
    shaderCanvas.fill(bgRgb.r, bgRgb.g, bgRgb.b, 255 * fScale);
    shaderCanvas.stroke(strokeRgb.r, strokeRgb.g, strokeRgb.b, 255 * fScale);
    shaderCanvas.strokeWeight(1.5); 
    
    // Smoothly scale down the voxel based on fluidity fade
    let drawS = s * map(fScale, 0.0, 1.0, 0.1, 1.0);

    for (let z = 0; z < n; z++) {
      shaderCanvas.push();
      shaderCanvas.translate(0, 0, z * cell + cell * 0.5);
      shaderCanvas.box(drawS, drawS, drawS);
      shaderCanvas.pop();
    }
